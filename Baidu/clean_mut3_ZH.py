...
def clean_mut3_ch_with_keep_original_brackets(mut3_text: str, keep_brackets: list) -> str:
    if not mut3_text:
        return ""

    cleaned_mut = clean_text_light(mut3_text)
...
def clean_mut3_ch_from_csv(csv_path: str, raw_zh_file: str, output_zh_file: str) -> list:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"CSV file does not exist£º{csv_path}")
    if not raw_zh_path.exists():
        raise FileNotFoundError(f"The original file of mut3 does not exist£º{raw_zh_path}")
    ...
    with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:
        ...
        if missing_cols:
            raise ValueError(f"Column name not found in CSV£º{missing_cols}£¬available columns: {reader.fieldnames}")
    ...
    with open(raw_zh_path, 'r', encoding='utf-8') as f:
        mut3_lines = [line.strip() if line else "" for line in f]


    if len(mut3_lines) != len(original_brackets_list):
        raise ValueError(f"The number of rows does not match! The CSV file has{len(original_brackets_list)}lines, while the mut3 file has{len(mut3_lines)}lines")


    cleaned_lines = []
    for idx, (mut_line, keep_brackets) in enumerate(zip(mut3_lines, original_brackets_list)):
        cleaned_line = clean_mut3_ch_with_keep_original_brackets(mut_line, keep_brackets)
...
def read_cleaned_zh_txt(txt_path: str) -> list:
    ...
    if not txt_path.exists():
        raise FileNotFoundError(f"The cleaned mut3 file does not exist:{txt_path}")
...
def add_mut3_to_csv(csv_path: str, zh_sentences: list, new_column: str = "cleaned_mut3_ch",
    ...
    if output_csv is None:
        output_csv = csv_path.parent / f"{csv_path.stem}_with_mut3{csv_path.suffix}"
    else:
    ...
    if csv_row_count != zh_row_count:
        raise ValueError(f"The number of rows does not match! CSV has {csv_row_count} rows, while mut3 TXT has {zh_row_count} rows")
...
if __name__ == "__main__":
    ...
    RAW_MUT3_ZH_FILE = "mut_translation_results/mut3_zh.txt"  

    CLEANED_MUT3_ZH_FILE = "bugDetectionData/cleaned_mut3_ch_sent.txt"  
    OUTPUT_CSV_WITH_MUT3 = "mut_translation_results/success_mutations.csv"

    NEW_MUT3_COLUMN = "cleaned_mut3_ch"


    cleaned_mut3_sentences = clean_mut3_ch_from_csv(
        csv_path=CSV_FILE,
        raw_zh_file=RAW_MUT3_ZH_FILE,
        output_zh_file=CLEANED_MUT3_ZH_FILE
    )


    add_mut3_to_csv(
        csv_path=CSV_FILE,
        zh_sentences=cleaned_mut3_sentences,
        new_column=NEW_MUT3_COLUMN,
        output_csv=OUTPUT_CSV_WITH_MUT3
    )
