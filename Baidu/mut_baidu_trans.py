...
# Execute extract_mutSuccess.py after translation
appid = ''
appkey = ''
from_lang = 'en'
to_lang = 'zh'
...
def createRequest(query):
    ...
    if len(q) > 6000:
        q = q[:6000]
        print(f"[Warning] Text too long, truncated to 6000 characters:{q[:50]}...")
    ...
    try:
        ...
        if 'error_code' in result:
            ...
            }.get(result['error_code'], f'error code{result["error_code"]}')
        ...
        with counter_lock:
            ...
            if request_counter % PRINT_INTERVAL == 0:
                print(f"Completed {request_counter} translation requests")

        return trans_text

    except Exception as e:
        print(f"[translation error] {str(e)} | Original text: {q[: 50]}")
        return q
...
def main():
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"The input file does not exist：{INPUT_TXT_PATH}")

    with open(input_path, 'r', encoding='utf-8') as f:
        en_sentences = [line.rstrip('\n') for line in f]
    total_sentences = len(en_sentences)
    print(f"Read completed, with a total of {total_sentences} lines of English sentences")
    ...
    with open(OUTPUT_CN_TXT_PATH, 'w', encoding='utf-8') as f:
        f.write('\n'.join(cn_sentences))
    print(f"Chinese TXT has been generated：{OUTPUT_CN_TXT_PATH}")
    ...
    with open(OUTPUT_MIX_TXT_PATH, 'w', encoding='utf-8') as f:
        f.write('\n'.join(mix_lines))
    print(f"Chinese English alternate TXT has been generated:{OUTPUT_MIX_TXT_PATH}")
    ...
    print(f"Successful translation: {success_num}")
    print(f"number of failures:{fail_num}")
    print(f"Total time:{total_time:.2f} seconds")
    print(f"Average speed:{total_sentences / total_time:.2f} lines/second")
    print(f"Success rate:{success_num / total_sentences * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    # INPUT_TXT_PATH = 'mutation_txt/mut1_cleaned.txt'
    # OUTPUT_CN_TXT_PATH = 'mut_translation_results/mut1_zh_pairs.txt'
    # OUTPUT_MIX_TXT_PATH = 'mut_translation_results/mut1_zh.txt'

    # INPUT_TXT_PATH = 'mutation_txt/mut2_cleaned.txt'
    # OUTPUT_CN_TXT_PATH = 'mut_translation_results/mut2_zh.txt'
    # OUTPUT_MIX_TXT_PATH = 'mut_translation_results/mut2_zh_pairs.txt'

    INPUT_TXT_PATH = 'mutation_txt/mut3_cleaned.txt'
    OUTPUT_CN_TXT_PATH = 'mut_translation_results/mut3_zh.txt'
    OUTPUT_MIX_TXT_PATH = 'mut_translation_results/mut3_zh_pairs.txt'
