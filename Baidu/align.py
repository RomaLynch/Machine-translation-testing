...
# after running this py file please run dataPre_for_mut.py
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
print(f"{DEVICE}")

LOCAL_MODEL_PATH = "bert-base-multilingual-cased"
if not os.path.exists(LOCAL_MODEL_PATH):
    raise FileNotFoundError(f"local path is not existed£º{LOCAL_MODEL_PATH}")

print(f"£º{LOCAL_MODEL_PATH}")
...
print("SimAlign finished")

en_tokenizer = tokenizer_13a.Tokenizer13a()
print("Tokenizer13a finished")
...
def batch_en_ch_align(
    ...
    assert len(en_sentences) == len(zh_sentences) == len(tgt_polys), \
        f"not same lines. EN: {len(en_sentences)}, ZH: {len(zh_sentences)}, POLY: {len(tgt_polys)}"
    ...
    for i in range(total):
        ...
        if (i + 1) % 5000 == 0:
            print(f" processed {i + 1} / {total} line")
...
if __name__ == "__main__":
    en_sents = read_lines_keep_empty("align_cleaned_corpus/cleaned_3w_um_src.txt")
    zh_sents = read_lines_keep_empty("align_cleaned_corpus/cleaned_3w_um_zh.txt")
    tgt_words = read_lines_keep_empty("corpus/tgt_word_cleaned.txt")

    print(f"Input file lines: EN: {len(en_dents)}, ZH: {len(zh_dents)}, POLY: {len(tgt_words)}")

    results = batch_en_ch_align(en_sents, zh_sents, tgt_words)

    output_csv = "aligned_results/align_res.csv"
    ...
    print(f"Alignment completed, result saved to: {outputting_csv}")
