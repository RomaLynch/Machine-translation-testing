...
def clean_mut1_ch_with_keep_original_brackets(mut1_text: str, keep_brackets: list) -> str:
    if not mut1_text:
        return ""

    cleaned_mut = clean_text_light(mut1_text)
...
def extract_column_from_csv(csv_path: str, output_txt_path: str, column_name: str) -> None:
    ...
    with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:  
        ...
        if column_name not in reader.fieldnames:
            raise ValueError(f"Column name {column_name} not found in CSV, available column names:{reader.fieldnames}")
    ...
    with open(output_txt_path, 'w', encoding='utf-8') as f:
        for value in column_values:
            f.write(value + '\n')
    print(f"   Extract column name:{column_name}")
    print(f"   Extract rows: {len(column_values)}")
...
def clean_file(input_filepath: str, output_filepath: str, use_gloss_clean: bool = False) -> None:
    if not os.path.exists(input_filepath):
        raise FileNotFoundError(f"The input file does not exist:{input_filepath}")
...
def clean_mut1_ch_from_csv(csv_path: str, raw_zh_file: str, output_zh_file: str) -> list:
    csv_path = Path(csv_path)
    ...
    if not raw_zh_path.exists():
        raise FileNotFoundError(f"The original file of the variant sentence does not exist：{raw_zh_path}")
    ...
    with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:  
        ...
        if missing_cols:
            raise ValueError(f"Column name not found in CSV: {missing_cols}，available columns:{reader.fieldnames}")
    ...
    with open(raw_zh_path, 'r', encoding='utf-8') as f:
        mut1_lines = [line.strip() if line else "" for line in f]
    if len(mut1_lines) != len(original_brackets_list):
        raise ValueError(f"The number of rows does not match! CSV has{len(original_brackets_list)}lines, and the variant sentence file has{len(mut1_lines)}lines")

    cleaned_lines = []
    for idx, (mut_line, keep_brackets) in enumerate(zip(mut1_lines, original_brackets_list)):
        cleaned_line = clean_mut1_ch_with_keep_original_brackets(mut_line, keep_brackets)
...
def read_cleaned_zh_txt(txt_path: str) -> list:
    ...
    if not txt_path.exists():
        raise FileNotFoundError(f"The cleaned Chinese TXT file does not exist:{txt_path}")
...
def add_zh_to_csv(csv_path: str, zh_sentences: list, new_column: str = "cleaned_mut1_ch",
    ...
    if output_csv is None:
        output_csv = csv_path.parent / f"{csv_path.stem}_with_zh{csv_path.suffix}"
    else:
    ...
    if csv_row_count != zh_row_count:
        raise ValueError(f"The number of rows does not match! CSV has  {csv_row_count}rows, while Chinese TXT has {zh_row_count} rows")
...
if __name__ == "__main__":
    ...
    RAW_ZH_FILE = "mut_translation_results/mut1_zh.txt"
    ...
    CLEANED_ZH_FILE = "bugDetectionData/cleaned_mut1_ch_sent.txt"
    OUTPUT_CSV_WITH_ZH = "mut_translation_results/success_mutations.csv"

    NEW_ZH_COLUMN = "cleaned_mut1_ch"
    extract_column_from_csv(
        csv_path=CSV_FILE,
        output_txt_path=EXTRACTED_EN_TXT,
        column_name="en_sent"  
    )

    extract_column_from_csv(
        csv_path=CSV_FILE,
        output_txt_path=EXTRACTED_POLYWORD_TXT,
        column_name="polyword"
    )

    clean_file(
        input_filepath=EXTRACTED_EN_TXT,
        output_filepath=CLEANED_EN_FILE,
        use_gloss_clean=False
    )

    cleaned_zh_sentences = clean_mut1_ch_from_csv(
        csv_path=CSV_FILE,
        raw_zh_file=RAW_ZH_FILE,
        output_zh_file=CLEANED_ZH_FILE
    )

    add_zh_to_csv(
        csv_path=CSV_FILE,
        zh_sentences=cleaned_zh_sentences,
        new_column=NEW_ZH_COLUMN,
        output_csv=OUTPUT_CSV_WITH_ZH
    )